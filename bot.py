...
def find_sheet_row(sheet, user_id: int):
    cached_row = SHEET_ROWS.get(user_id)
    if cached_row:
        return cached_row

    try:
        ids = sheet.col_values(2)  # колонка B: ID клиента
        for index, value in enumerate(ids, start=1):
            if str(value).strip() == str(user_id):
                SHEET_ROWS[user_id] = index
                return index
    except Exception as e:
        print("Ошибка поиска строки в таблице:", e)
